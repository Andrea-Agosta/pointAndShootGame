...
class Raven {
  ...
  update(deltaTime) {
    (this.y < 0 || this.y > canvas.height - this.height) &&  (this.directionY = this.directionY * -1);
    this.x -= this.directionX;
    this.y += this.directionY;
    (this.x < 0 - this.width) && (this.markedForDeletion = true);
    ...
    (this.x < 0 - this.width) && (gameOver = true);
  };
  draw() {
...
class Particle {
    ...
    update(){
        this.x += this.speedX;
        this.radius += 0.3;
        (this.radius > this.maxRadius - 5) && (this.markedForDeletion = true);
    }
    draw(){
        ctx.save();
...
window.addEventListener('click', function (e){
    ...
    ravens.forEach(objects => {
        if(objects.randomColors[0] === pc[0] && objects.randomColors[1] === pc[1] && objects.randomColors[2] === pc[2]){
            // collision detected
